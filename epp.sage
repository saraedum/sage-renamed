...
###vx = RationalFunctionFieldValuation(K, GaussValuation(QQ['x'], pAdicValuation(QQ, 2)))
###
###R.<y> = K[]
####L.<y> = K.extension(y^2+y+x)
###L.<y> = K.extension(y^2+x)
###vy = vx.mac_lane_approximants(L.polynomial())
###assert len(vy)==1
###vy = FunctionFieldPolymodValuation(L, vy[0])
###
#### sanity check
###for i in range(256):
###    _y = L.random_element()
...
####G = t^4+2*t^2+4*x
###G = t^4 + 64*t + 4*x
###w0 = GaussValuation(R, vy)
###w1 = w0.extension(t, 1/2)
###
###
###
###
